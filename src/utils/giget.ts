...
async function getTemplateWithGiget(template: string) {
    ...
    const selectedPackageManagerName = await getUserCurrentPackageManagerFromPrompt()
    // @ts-ignore
    ...
    try {
        ...
        dependenciesCommand = await checkAndInstallPackages(selectedModuleTemplateConfig)
